...
void SnakeActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "Snake");
  ...
  if (gameOver) {
    renderer.drawCenteredText(UI_12_FONT_ID, y0 + gridH / 2 - 20, "GAME OVER", true, EpdFontFamily::BOLD);
    renderer.drawCenteredText(UI_10_FONT_ID, y0 + gridH / 2 + 10, "Press OK to restart");
  } else if (!running) {
    renderer.drawCenteredText(UI_12_FONT_ID, y0 + gridH / 2, "PAUSED", true, EpdFontFamily::BOLD);
  }

  const auto labels = mappedInput.mapLabels(tr(STR_BACK), gameOver ? "New" : tr(STR_SELECT), tr(STR_DIR_UP),
                                            tr(STR_DIR_DOWN));
