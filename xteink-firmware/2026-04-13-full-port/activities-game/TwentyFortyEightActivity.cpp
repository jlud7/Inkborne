...
void TwentyFortyEightActivity::render(RenderLock&&) {
  ...
  if (won) {
    renderer.drawCenteredText(UI_12_FONT_ID, y0 + grid + 40, "You reached 2048!", true, EpdFontFamily::BOLD);
  } else if (gameOver) {
    renderer.drawCenteredText(UI_12_FONT_ID, y0 + grid + 40, "GAME OVER", true, EpdFontFamily::BOLD);
  }

  const auto labels = mappedInput.mapLabels(tr(STR_BACK), (won || gameOver) ? "New" : "", "", "");
