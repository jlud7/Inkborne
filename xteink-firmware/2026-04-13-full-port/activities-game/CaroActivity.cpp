...
void CaroActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "Gomoku");
  ...
  const auto labels = mappedInput.mapLabels(tr(STR_BACK), "Place", tr(STR_DIR_UP), tr(STR_DIR_DOWN));
