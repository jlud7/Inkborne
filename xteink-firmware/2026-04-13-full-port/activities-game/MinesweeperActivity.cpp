...
void MinesweeperActivity::render(RenderLock&&) {
  ...
  if (lost) {
    renderer.drawCenteredText(UI_12_FONT_ID, y0 + boardH + 8, "GAME OVER", true, EpdFontFamily::BOLD);
  } else if (won) {
    renderer.drawCenteredText(UI_12_FONT_ID, y0 + boardH + 8, "You win!", true, EpdFontFamily::BOLD);
  }

  const auto labels = mappedInput.mapLabels(tr(STR_BACK), "Reveal", tr(STR_DIR_UP), tr(STR_DIR_DOWN));
