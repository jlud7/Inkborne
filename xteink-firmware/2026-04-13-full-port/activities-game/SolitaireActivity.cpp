...
const char* SolitaireActivity::suitSymbol(uint8_t suit) {
  static const char* kSymbols[] = {"H", "D", "C", "S"};
...
void SolitaireActivity::drawCard(int x, int y, int w, int h, const Card& card, bool sel, bool faceDown) {
...
void SolitaireActivity::render(RenderLock&&) {
  ...
  if (!waste.empty()) {
    ...
    renderer.drawRect(wasteX, topStartY, cardW, cardH, true);
  }
  ...
  for (int i = 0; i < FOUNDATION_COUNT; ++i) {
    ...
    if (foundation[i].rank == 0) {
      renderer.drawRect(fx, topStartY, cardW, cardH, true);
  ...
  for (int column = 0; column < TABLEAU_COUNT; ++column) {
    ...
    if (tableau[column].empty()) {
      renderer.drawRect(x, tableauStartY, tableauCardW, tableauCardH, true);
      if (column == selectedColumn && selectionMode == 0) {
        renderer.drawRect(x + 2, tableauStartY + 2, tableauCardW - 4, tableauCardH - 4, true);
      }
  ...
  if (gameWon) {
    ...
    renderer.drawCenteredText(UI_10_FONT_ID, statusY + 18, "Press OK for new game");
  } else if (gameLost) {
  ...
  const auto labels = mappedInput.mapLabels("Back", (gameWon || gameLost) ? "New" : "Play", "Auto", "Draw");
